...
public class TokenAuthenticationConverter implements ServerAuthenticationConverter {
	...
	public Mono<Authentication> convert(ServerWebExchange exchange) {
		...
		if (authorizationToken == null || !authorizationToken.startsWith(BEARER_TOKEN_PREFIX)) {
			return Mono.empty();
		}

		return Mono.just(new JWTAuthenticationToken(authorizationToken.substring(BEARER_TOKEN_PREFIX.length())));
	}
}
